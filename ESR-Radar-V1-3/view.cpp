...
View::View()
{
    ...
    for(int i = 0 ; i < MAX_TRACK_SIZE ; i++){
        ...
        track[i].Y = 0;
        if(i%1==0){
            track_color[i] = Scalar(random(100,200),random(50,100),random(50,100));
        }
        if(i%2==0){
            track_color[i] = Scalar(random(50,100),random(100,200),random(50,100));
        }
        if(i%3==0){
            track_color[i] = Scalar(random(50,100),random(50,100),random(100,200));
        }
    }
...
void View::paint_object_trajectory_track(int i , int flag){
    ...
    if(flag == TRACK_VIEW_TRAJECTORY){
        if(vector_track_movement == TRACK_STATIC){
            ...
            if( (int)vector_track[i].size() > data_measurement.min.Persistence){
                //연속되는 점 설정
    ...
    if(flag == TRACK_VIEW_BLOB){
        ...
        if(i == MAX_TRACK_SIZE-1){
            ...
            params.minArea = 10.0f-(float)zoom.level;//최소 영역 범위
            params.maxArea = 12000.0f;//최대 영역 범위
    ...
    if(flag == TRACK_VIEW_AUTOMATIC_CLUSTERING){
        if(i == 0){
            ...
            //자동 클러스터링 시작
            float distance = (float)data_measurement.max.AutomaticClusteringDistance/100.0*2.0;
            int clust_id = 0;
            bool check;
            for(int i = 0; i < (int)vector_cluster.size() ; i++){
                check = false;
                for(int j = i ; j < (int)vector_cluster.size() ; j++){
                    //기준점 x,y 설정
                    x1 = vector_cluster.at(i).x;
                    y1 = vector_cluster.at(i).y;
                    x2 = vector_cluster.at(j).x;
                    y2 = vector_cluster.at(j).y;
                    if(vector_cluster.at(j).id == TRACK_DEFAULT){
                        if(cal_point_distance(x1,y1,x2,y2)<= distance ){
                            vector_cluster.at(j).id = clust_id;
                            vector_cluster.at(j).color = track_color[clust_id];
                            check = true;
                        }

                    }else{
                        if(cal_point_distance(x1,y1,x2,y2)<= distance ){
                            vector_cluster.at(j).id = vector_cluster.at(i).id;
                            vector_cluster.at(j).color = vector_cluster.at(i).color;
                        }
                    }
                }
            ...
            vector<Cluster> vector_center_cluster;
            float cx,cy;
            int count;
            Scalar color;
            qDebug()<<frame<<" " <<vector_cluster.size();
            ...
            for(int j = 0; j < (int)vector_cluster.size() ; j++){
                ...
                count = 0;
                for(int i = 0; i < (int)vector_cluster.size() ; i++){
                    if(vector_cluster.at(i).id == j){
                        cx += vector_cluster.at(i).x;
                        cy += vector_cluster.at(i).y;
                        count++;
                        color = track_color[j];
                    }
                }

                center_cluster.x = cx/count;
                center_cluster.y = cy/count;
                center_cluster.color = color;
                center_cluster.id = j;

                vector_center_cluster.push_back(center_cluster);
            }

            //색칠
            for(int i = 1; i < (int)vector_center_cluster.size() ; i++){//0은 초창기 값 이여서 nan임

                x2 = vector_center_cluster.at(i).x;
                y2 = vector_center_cluster.at(i).y;

                for(int j = 0; j < (int)vector_cluster.size() ; j++){

                    if(vector_center_cluster.at(i).id == vector_cluster.at(j).id){
                        x1 = vector_cluster.at(j).x;
                        y1 = vector_cluster.at(j).y;
                        ellipse(mat_map,RotatedRect( Point2f( check_track_x_axis(x1),check_track_y_axis(y1) ), Size2f(PIXEL_RADUATION_SIZE/(REAL_RADUATION_SIZE-zoom.level)*distance,PIXEL_RADUATION_SIZE/(REAL_RADUATION_SIZE-zoom.level)*distance),PIXEL_RADUATION_SIZE ),vector_cluster.at(j).color);

                        Scalar tmp = vector_cluster.at(j).color;

                        line(mat_map,Point2f(check_track_x_axis(x1),check_track_y_axis(y1)),Point2f(check_track_x_axis(x2),check_track_y_axis(y2)),Scalar(0,255,255));
                    }
                }

                ellipse(mat_map,RotatedRect(Point2f(check_track_x_axis(x2),check_track_y_axis(y2)) ,Size2f(TRACK_SIZE,TRACK_SIZE),TRACK_SIZE ),Scalar(255,255,255));
...
void View::cal_object_trajectory_vector_track(int i){
    ...
    if(vector_track_movement==TRACK_STATIC){
        if(vector_track[i].size() > data_measurement.max.MaxSPMBuf){
