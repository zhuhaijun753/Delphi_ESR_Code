...
typedef struct{
    ...
    float Cohesion;//Obstacle Area : 장애물 추출 사각영역
    int Persistence;//Obstacle Accuracy : 백터 크기(민감도)
    float CriticalSpeed;//장애물 속도가 일정 이하이면 무시
    int Viability;//깜빡인것 허용치 최대 범위(size에 따른 %임 예를들어 10개의 size를 vector에다가 입력해서 그중 10%라 하면 1개의 깜빡임은 눈감아 주는것임)(값이 높을수록 깜빡인것도 장애물로 인정, 값이 낮을수록 깜빡인것은 장애물로 불인정)
    int AutomaticClusteringDistance;//Automatic Clustering Distance;
    int kNN;//k-Nearest Neightbor;
    int BlobArea;//blob 영역 최소,최대 크기
    int BlobDistance;//blob 원영역 거리 : 좌표를 통해서만 추출하면 하나의 점이 됨으로 일정 거리를 두어 영역으로 표시(원)
    int BlobMinDistance;//이웃 blob까지의 최소 거리에 포함되면 하나의 영역으로 그룹화
    int CCLArea;//ccl 영역 최소,최대 크기
    int CCLMode;//ccl 탐색 기준 설정 4방향,8방향 선택
    int MaxSPMBuf;//최대로 쌓을 백터 크
}LimitData;
